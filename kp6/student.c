...
//инициализация таблицы пустыми значениями
...
//получение хеша по ключу (через умножение на простое число)
...
void read_from_str(student *s, char *line) {
    sscanf(line, "%s %s %s %d %c %d", s->surname, s->initials, s->gender, &(s->class_num), &(s->index),
           &(s->school_num));
}
...
int read_string_bin(char *str, FILE *in) {
    ...
    if (length > 0) {
        if ((int)fread(str, sizeof(char), length, in) != length) {
            *str = '\0';  // В случае ошибки чтения, устанавливаем пустую строку
            return 0;     // Возвращаем 0, чтобы указать на ошибку чтения
        }
    }
...
// Чтение с бинарника в структуру
...
void print_table(hash_table *table) {
    ...
    for (int i = 0; i < TABLE_SIZE; i++) {
        ...
        printf("| %15s | %10s |    %2d-%c     | %18d |\n", tmp->surname, tmp->initials, tmp->class_num, tmp->index,
               tmp->school_num);
    }
    printf("|-----------------------------------------------------------------|\n");
}

// Вставка студента из файла в конец другого файла
void insert_student(const char *filename, char *in, hash_table *table) {
    ...
    if (found_student(table, key)) {
        printf("Студент уже в базе данных\n");
...
// Удаление студента и занесение данных в таблицу по ключу
void erase_student(const char *filename, const char *key, hash_table *table) {
    if (!found_student(table, key)) {
        printf("Школьника нет в базе данных\n");
        return;
    }
