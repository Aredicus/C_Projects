...
typedef struct cell {
    ...
    student *data; // значение-ученик
} cell;

// структура хеш-таблицы
typedef struct hash_table {
    cell cells[TABLE_SIZE]; //массив элементов
} hash_table;

// функции чтения и записи
int read_student_bin(student *s, FILE *in); //чтение бинарника (всей записи)

void write_student_bin(student *s, FILE *in); //запись бианрника (всей записи)

int read_string_bin(char *str, FILE *in); //

void write_string_bin(const char *str, FILE *out); //

void write_student_txt(student *s, FILE *in); //запись txt (всей записи)

void read_from_str(student *s, char *line); //


// функции для значения
